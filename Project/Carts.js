...
class Carts {
  constructor(products, containerElement) {
    ...
    this._showCartPopup = false; //ถ้าว่างเปล่าก็ไม่ให้มี class ขึ้นมา
    // Element
  ...
  showHidePopUp() { //check false true
  ...
  summaryCalculator = () => {
    if (this._carts.length === 0) {
      this._summary_price.textContent = ''; //ว่างเปล่า
    } else {
      const priceSum = this._carts.reduce((prev, current) => { //price
        return prev + current.productDetails.Price * current.amount;
      }, 0);
      const TotalSum = this._carts.reduce((prev, current) => { //dishes current คือค่าปัจจุบันที่ชี้อยู่
  ...
  addToCart(id) { //รับค่าเป็นไอดีจากคลาสโปรดัก
    const indexFoodCart = this._carts.findIndex( //เช็คว่ามีของเดิมใน array มั้ย ถ้าไม่มีจะเป็น -1
      (e) => e.productDetails.ID === id
    );
    if (indexFoodCart === -1) {
      this._carts.push({
        productDetails: this.products.find((e) => e.ID === id), //find (id) ว่ามีใน localstorage ว่ามีมั้ย
        amount: 1, //กดครั้งแรกเป็น 1
      });
    } else {
    ...
    this.render();
    removeLocalStorage('carts'); //ลบอันเก่าแล้ว set ใหม่
