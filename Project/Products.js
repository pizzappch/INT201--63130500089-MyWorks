...
class Products { 
  constructor(products, containerElement, searchElement, searchBtnElement) {
    ...
    this._searchBtnElement.addEventListener('click', () => {
      ...
      this.render(); //เรียกเพราะ filter สิ่ง่ที่เสิร์ชแล้วเคลียร์ของเดิมแล้ว array ใหม่ เคลียร์เพื่อให้ของเก่ามันออกจะได้ใส่ของใหม่
    });
  ...
  render() {
    ...
    this.clearProductsRender(); //ต้องเคลียร์ child ออกให้หมดเพื่อ render
    for (const food of foodFilter) {
      ...
      btn.addEventListener('click', () => this._cart.addToCart(food.ID)); //ใช้ id filter เพื่อแยกแยะว่า product ไหน
